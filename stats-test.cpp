...
TEST_CASE("reports average, minimum and maximum for single number") {
    float numberset[] = {1.5};
    int setlength = sizeof(numberset) / sizeof(numberset[0]);
    struct Stats computedStats = compute_statistics(numberset, setlength);
    float epsilon = 0.001;
    REQUIRE(abs(computedStats.average - 1.5) < epsilon);
    REQUIRE(abs(computedStats.max - 1.5) < epsilon);
    REQUIRE(abs(computedStats.min - 1.5) < epsilon);
}
...
TEST_CASE("average is NaN for negative size") {
    Stats computedStats = compute_statistics(0, -6);
    //All fields of computedStats (average, max, min) must be
    //NAN (not-a-number), as defined in math.h
